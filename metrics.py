...
def make_subset_loader(conf: TrainConfig,
                       ...
                       parallel: bool,
                       drop_last=True):
    if len(dataset) > conf.eval_num_images:
...
def evaluate_lpips(
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_loader, desc='lpips'):
            imgs = batch[conf.dataset_img_key].to(device)
            ...
            if use_inverted_noise:
                ...
                x_T = torch.randn((len(imgs), conf.in_channels, conf.img_size, conf.img_size),
                                  device=device)
...
def evaluate_fid(
    ...
    if get_rank() == 0:
        ...
        if not os.path.exists(cache_dir):
            ...
            loader_to_path(val_loader, cache_dir, denormalize=True, img_key=conf.dataset_img_key)
    ...
    with torch.no_grad():
        if conf.model_type.can_sample():
            ...
            for i in trange(0, eval_num_images, batch_size, desc=desc):
                ...
                x_T = torch.randn(
                    (batch_size, conf.in_channels, conf.img_size, conf.img_size),
                    device=device)
            ...
            if conf.train_mode.is_latent_diffusion():
                ...
                for batch in tqdm(train_loader, desc='generating images'):
                    imgs = batch[conf.dataset_img_key].to(device)
                    x_T = torch.randn(
                        (len(imgs), conf.in_channels, conf.img_size, conf.img_size),
                        device=device)
...
def loader_to_path(loader: DataLoader, path: str, denormalize: bool, img_key: str):
    # not process safe!
