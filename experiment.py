...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        self.register_buffer(
            'x_T',
            torch.randn(conf.sample_size, conf.in_channels, conf.img_size, conf.img_size))
        ...
        with writer:
            ...
            for batch in tqdm(loader, total=len(loader), desc='infer'):
                with torch.no_grad():
                    ...
                    cond = model.encoder(batch[self.conf.dataset_img_key].to(self.device))
                    '''
            ...
            with torch.no_grad():
                ...
                if save_real and use_xstart:
                    ...
                    if self.global_rank == 0:
                        ...
                        ndarr = grid_real.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to("cpu", torch.uint8).numpy()
                        self.logger.log_image(f'sample{postfix}/real', [Image.fromarray(ndarr)])
                ...
                if self.global_rank == 0:
                    ...
                    self.logger.log_image(f'sample{postfix}', [Image.fromarray(ndarr)])
            model.train()
