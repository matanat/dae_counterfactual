...
class CropMaskByLabel(monai.transforms.Transform):
...
class AssertEmptyImaged(monai.transforms.Transform):
    def __init__(self):
        super().__init__()
    def __call__(self, data):
        d = dict(data)

        assert d["image"].size()[-1], "patient %d, label %d, %s, %s" % (d['patient'], d['ivd_label'], str(d["image"].size()), str(d["mask"].size()))
