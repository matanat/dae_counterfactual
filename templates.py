...
def brats64_autoenc():
    conf = ffhq64_autoenc()
    conf.data_name = 'brats64'
    conf.eval_every_samples = 10_000_000
    conf.eval_ema_every_samples = 10_000_000
    conf.total_samples = 72_000_000
    conf.name = 'brats64_autoenc'
    conf.dataset_img_key = "image"
    # brats is B&W single channgel
    conf.in_channels = 1
    conf.out_channels = 1
    return conf

def spider64_autoenc():
    conf = ffhq64_autoenc()
    conf.data_name = 'spider64'
    conf.eval_every_samples = 10_000_000
    conf.eval_ema_every_samples = 10_000_000
    conf.total_samples = 72_000_000
    conf.name = 'spider64_autoenc'
    conf.dataset_img_key = "image"
    # spider is B&W single channgel
    conf.in_channels = 1
    conf.out_channels = 1
    return conf
...
def retina128_autoenc_130M():
    conf = retina128_autoenc_base()
    conf.total_samples = 130_000_000
    conf.eval_ema_every_samples = 10_000_000
    conf.eval_every_samples = 10_000_000
    conf.name = 'retina128_autoenc_130M'
    return conf
...
def pretrain_brats64_72M():
    conf = brats64_autoenc()
    conf.pretrain = PretrainConfig(
        name='72M',
        path=f'checkpoints/{brats64_autoenc().name}/last.ckpt',
    )
    conf.latent_infer_path = f'checkpoints/{brats64_autoenc().name}/latent.pkl'
    return conf
...
def pretrain_retina128_autoenc130M():
    conf = retina128_autoenc_base()
    conf.pretrain = PretrainConfig(
        name='130M',
        path=f'checkpoints/{retina128_autoenc_130M().name}/last.ckpt',
    )
    conf.latent_infer_path = f'checkpoints/{retina128_autoenc_130M().name}/latent.pkl'
    return conf
